...
当前工作区版本：`1.2.0`（Android `versionCode` 4）。相较上游 `v1.1.0`，本版本保留了本地的明亮界面，并进一步优化了播放模式切换。
...
当前工程包含 52 项自动化测试：24 项 JVM 单元测试覆盖 LRC 解析与资源边界、原子文件替换/降级、播放策略和歌单竞争校验；28 项 Android 集成测试覆盖 MediaStore 映射、Media3 服务与冷启动控制链、Room 迁移/约束/文件单例、ViewModel 状态边界、歌词持久化/并发、权限流程和歌词导入目标绑定。Debug/Release APK 全量构建、Android 测试源码编译与 Android Lint 均纳入发布验证；设备端测试仍需连接模拟器或真机后执行。
