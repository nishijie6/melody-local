...
4. 从歌词菜单可编辑原始 LRC、插入当前时间、整体平移时间轴、替换或移除歌词；也可开启悬浮歌词、通知/锁屏歌词及输出延迟校准。
...
当前工程包含 121 项自动化测试：74 项 JVM 单元测试覆盖 LRC/增强 LRC、多语分层、内嵌标签、同目录及 LRCLIB 匹配、歌词编辑、输出路由与延迟策略，以及原有播放、歌单和媒体操作规则；47 项 Android 集成测试覆盖歌词持久化与自动发现抑制、系统歌词设置、MediaStore、Media3、Room 迁移、ViewModel 竞态、真实 Transformer 输出和播放服务后台发现。CI 在 API 26、29、36 模拟器执行设备测试。
